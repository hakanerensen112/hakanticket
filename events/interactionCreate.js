...
module.exports = async (client, int) => {
    ...
    switch (req) {
        case 'createTicket': {
            ...
            selectMenu.setPlaceholder('Destek İçin Bir Sebep Seçiniz');
        ...
        case 'newTicket': {
            ...
            if (!channel) {
                ...
                ticketEmbed.setAuthor(` <@&898214566968840203>, <@&898214568357142548>, <@&898214571364450306> \n Desteğiniz başarıyla oluşturuldu ${int.member.user.username}${reason ? ` (${reason})` : ''} ✅`);
                ...
                return int.update({ content: `Talebiniz Başarıyla Açıldı <@${int.member.id}> <#${channel.id}> ✅`, components: [], ephemeral: true });
            } else {
        ...
        case 'closeTicket': {
            ...
            ticketEmbed.setAuthor(`${int.member.user.username} Desteği Kapatmaya Çalışıyor ❌`);
            ...
            reopenButton.setLabel('Yeniden Destek Oluştur');
        ...
        case 'saveTicket': {
            ...
            await channel.messages.fetch().then(async msg => {
                let messages = msg.filter(msg => msg.author.bot !== true).map(m => {
                    ...
                    const user = `${m.author.tag}${m.author.id === int.customId.split('_')[1] ? ' (Destek İsteyen Kullanıcı)' : ''}`;
                ...
                stream.once('open', () => {
                    stream.write(`Kullanıcı Destek ${int.customId.split('_')[1]} (Kanal #${channel.name})\n\n`);
                    stream.write(`${messages}\n\nLoglar ${new Date(ticketID).toLocaleString()}`);
